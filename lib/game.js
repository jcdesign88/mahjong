...
class Game {
  ...
  requestJoin(socketId, name, rejoinToken) {
    ...
    if (this.phase !== "lobby") {
      return { ok: false, error: "对局已开始。若你刚才掉线，请用同一设备打开原来的房间链接重连" };
    }
  ...
  approveJoin(hostSocketId, targetSocketId) {
    ...
    if (this.phase !== "lobby") return { ok: false, error: "已开局" };
    ...
    // Prefer truly empty seat, else take a bot seat (not reserved reconnect seats)
    let seat = this.players.findIndex((p) => !p.id && !p.isBot && !p.rejoinToken);
    if (seat < 0) seat = this.players.findIndex((p) => p.isBot && !p.id);
    if (seat < 0) {
      return { ok: false, error: "座位已满 — 请先踢掉一位机器人、离线座位或玩家" };
    }
    ...
    p.ready = false;
    p.rejoinToken = makeToken();
    this.addLog(`房主批准 ${p.name} 加入（${SEAT_NAMES[seat]}）`);
  ...
  kick(hostSocketId, seat) {
    ...
    if (this.phase !== "lobby") return { ok: false, error: "开局后不能踢人" };
    seat = Number(seat);
  ...
  leave(socketId) {
    ...
    // Keep rejoinToken so they can come back
    p.id = null;
    p.ready = false;
    p.paused = false;
    ...
    if (this.phase === "lobby") {
      ...
      if (seat === this.hostSeat) this.transferHostIfNeeded();
    } else {
      p.isBot = true;
      p.name = `${name || SEAT_NAMES[seat]}（离线）`;
      this.addLog(`${name} 掉线 — 机器人暂代（可重连）`);
      this.clearTurnTimer();
      // Keep hostSeat during a match so creator can reclaim host on rejoin
    }
  }
  ...
  viewFor(socketId) {
    ...
    return {
      ...
      // Other connected humans (bots have no socket id — voice is human-to-human only)
